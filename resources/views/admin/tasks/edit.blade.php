    ...
    <main class="w-full flex-grow p-6">
        ...
        <div class="bg-white border border-4 rounded-lg shadow relative m-10">
            ...
            <div class="p-6 space-y-6">
                <form id="form"  action="{{route('tasks.update', ['task'=> $task->id])}}" method="POST">
                    @method('put')
                    @csrf
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6 sm:col-span-3">
                            <label for="product-name" class="text-sm font-medium text-gray-900 block mb-2">Task Name</label>
                            <input type="text" name="name" id="name" value="{{old('name', $task->name)}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5"  required="">
                        </div>
                        <div class="col-span-6 sm:col-span-3">
...
{{--                        </div>--}}
                        <div class="col-span-full">
                            ...
                            <textarea id="description" name="description" value="{{old('description')}}}" required rows="6" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-4">{{$task->description}}</textarea>
                        </div>
                    </div>


                    <div class="p-6 border-t border-gray-200 rounded-b">
                        <button class=" bg-sidebar text-white focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="submit">update</button>
                    </div>
                </form>
            </div>
