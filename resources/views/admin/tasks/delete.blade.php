            ...
            @can('delete task')
                @if(@auth()->user()->id == $task->user_id | @auth()->user()->hasRole('admin'))
                    <div class="bg-white border border-4 rounded-lg shadow relative m-10">
                        ...
                        <div class="p-6 space-y-6">
                            <form id="form"  action="{{route('tasks.destroy', ['task'=> $task->id])}}" method="POST">
                                @method('DELETE')
                                @csrf
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="product-name" class="text-sm font-medium text-gray-900 block mb-2">Task Name</label>
                                        <input type="text" name="name" id="name" value="{{old('name', $task->name)}}" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" disabled  required="">
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="category" class="text-sm font-medium text-gray-900 block mb-2">Label</label>
                                        <select disabled class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" name="label_id" id="label_id">
                                            @foreach($labels as $label)
                                                <option value="{{ $label->id }}" @if($label->id == old('label_id', $task->label_id)) selected @endif>{{ $label->name }} </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="brand" class="text-sm font-medium text-gray-900 block mb-2">user</label>
                                        <select disabled class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" name="user_id" id="user_id">
                                    ...
                                    <div class="col-span-full">
                                        ...
                                        <textarea disabled id="description" name="description" value="{{old('description')}}}" required rows="6" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-4">{{$task->description}}</textarea>
                                    </div>
                                </div>
                ...
                @else
                    <div class="flex flex-col">
                        ...
                        <p class="text-2xl font-medium text-gray-600 mb-6">you are trying to edit a task that isn't yourse</p>
                    </div>
                    </body>
